 ...
 */
 window.addEventListener('DOMContentLoaded', () => {
	...
	readAllReplies(); // 포스트 상세 페이지가 로딩된 후 댓글 목록을 화면 출력
	...
	function updateReplyList(data) {
		...
		for (let r of data) {
			str += '<div class="card my-2">'
				...
				+ `<button type="button" class="btnModifies btn btn-outline-primary" data-rid="${r.replyId}">수정</button>` // 숫자 1번 왼쪽 ` 
				+ '</div>'
				+ '</div>'	
		}
		divReplies.innerHTML = str;
		
		// [수정] 버튼들의 HTML 요소로 만들어진 이후에, [수정] 버튼에 이벤트 리스너를 등록.
	...
	function getReply(event) {
		...
		// console.log(event.taget); // 이벤트가 발생한 타겟 -> 버튼
	...
	function deleteReply(event) {
		...
		if (result) {
			...
			.then(response => { // 성공(HTTP 200 OK) 응답
	...
	function updateReply(event) {
		...
		if (result) {
			...
			axios
			.put('/api/reply/' + replyId, data) // Ajax PUT 요청 전송
			.then(response => {
