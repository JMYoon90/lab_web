...
public class SecurityConfig {
	...
	// 암호화(복호화) 알고리즘 객체 -> Spring Security 에서는 비밀번호는 반드시 암호화를 해야 함.
	public InMemoryUserDetailsManager inMemoryUserDetailsManager() {
		UserDetails user1 = User.builder()
				.username("user1") // 로그인 아이디
				.password(passwordEncoder().encode("1111")) // 로그인 패스워드 - 반드시 암호화!
	...
	*/
	...
	public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
		...
		// 기능 구현을 간단히 하기 위해서 CSRF 기능을 비활성화.
		http.csrf().disable();
		...
		// - 익명사용자(로그인 하지 않은) 사용자도 접근할 수 있는 경로를 설정
...
//			.permitAll(); // 로그인 여부와 상관 없이 허용. 
		...
		// -> 컨트롤러 메서드를 작성할 때 에너테이션을 사용해서 권한 설정을 할 수 있음.
		// -> (1) SecurityConfig 클래스에는 @EnableGlobalMethodSecurity 에너테이션을 사용.
		// -> (2) 각각의 컨트롤러 메서드에 @PreAuthorize 또는 @PostAuthorize 에너테이션을 사용.
